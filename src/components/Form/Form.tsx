...
function Form() {
  ...
  const [aliveValue, setAliveValue] = useState('si'); 
  ...
  const handleButtonClick = () => {
    ...
    if (Country === "Argentina" && aliveValue=="si" &&((param1) || (typeCarta) || ((type==="audiovisual")&& (year - dYear >= 50))||typeCarta ||typeEmision ) ) {
      setComponentToShow(<IsPublicDomain title={nameObra} autor={nameAutor} />);
    } else if (Country !== "Argentina"  ) {
      setComponentToShow(<UnknowStatus title={nameObra} autor={nameAutor} />);
    } else {
      setComponentToShow(<NotPublicDomain title={nameObra} autor={nameAutor}/>);
    }
  };
  ...
  <div className="flex gap-2">
    ...
    <label className="text-center text-lg font-bold" >Fallecido</label>
    ...
    <div className="flex flex-col mb-6 gap-3 w-[50%]" >
    <label className="text-center text-lg font-bold" >Fecha deceso</label>
    <div
    ...
    <input
      type="date" name="deathDate" value={deathDate} onChange={(e) => setdeathDate(e.target.value)}
   ...
   <div className="flex gap-2">
   <div className="flex flex-col mb-6 gap-3 w-[50%]" >
    <label className="text-center text-lg font-bold">Fecha publicación</label>
    ...
    <select required  className="bg-gray-50 border border-gray-500 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" onChange={(e) => setCountry(e.target.value)}>
      ...
      {countries.map((item,index)=>(
        ...
        <option value={item.name.common} className="" key={index}>{item.flag} {item.name.common} </option>
      ))}
